<#############################################################
 ...
 #                                                           #
 #############################################################>
Import-LocalizedData -BindingVariable lLITxt -FileName AzStackHci.LocalIdentity.Strings.psd1
Import-Module $PSScriptRoot\AzStackHci.LocalIdentity.Helpers.psm1 -DisableNameChecking -Global

function Invoke-AzStackHciLocalIdentityValidation
{
    <#
    .SYNOPSIS
        Perform AzStackHci Local Identity Validation
    .DESCRIPTION
        Perform AzStackHci Local Identity Validation
    .EXAMPLE
        Example 1
    .PARAMETER LocalAdminUsername
        Only for Local Identity scenario, specify the local administrator username to be used for deployment.
    .PARAMETER PsSession
    ...
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true, HelpMessage = "Only for Local Identity scenario, specify the local administrator username to be used for deployment.")]
        [string]
        $LocalAdminUsername,
        ...
        [ArgumentCompleter({ Get-TestListByFunction -ModuleName AzStackHci.LocalIdentity.Helpers })]
        [ValidateScript({ $_ -in (Get-TestListByFunction -ModuleName AzStackHci.LocalIdentity.Helpers) })]
        [string[]]
        $Include,
    ...
    try
    {
        ...
        $testList = Get-TestListByFunction -ModuleName AzStackHci.LocalIdentity.Helpers
        ...
        $i = 0
        $Result = @()
        $ProgressActivity = "Checking AzStackHci LocalIdentity Compatibility"
        if ($PsSession)
        {
        ...
        if (-not $PassThru)
        {
            Write-Host 'LocalIdentity Results'
            Write-AzStackHciResult -Title 'LocalIdentity' -Result $Result
    ...
    finally
    {
        ...
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 'LocalIdentity' -Value $Result -Force
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD18Eaa6UNVf5Av
# X11tgfM8lhIUaj9OrOQ1ipBwXKm2T6CCDMkwggYEMIID7KADAgECAhMzAAACHPrN
...
# hvcNAQkEMSIEIAkkODcF6ZS3G3EM4qTWL3rfrxR0sFgyimNP+SZbAXixMEIGCisG
# AQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3
# Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEBBQAEggEAZPhsej+WVDRv8ETyIdGf
# /lMyWKLIWOKc2xdHKVnZvqW7YIuz4wQ16BnbsRpDoq5+UfpDAgzk4xIQ1Up/qQqZ
# V+3H+QP0QJBidzoDMlTfi7ATYwScK9mcbFjmXyjnHB62NO2R+4915qhQa++w2zs3
# OScjpfVdCXE7f9HN0xVMVdLk/S4svZKbUVPPGc6zIPncKgAxx7zTS98toC3ioPqd
# q8v+2k/qX+Gj3DO4BepG7HbLWudmpxnZ7yHqaCovJDqKTyafsWg5h8upLglIAEim
# n85RW/6vW97zV7zr19t90VhlXcDhGbt5YrWvzCOdVR6AYD5P2U0rIdMbi4EZZoeR
# HKGCF60wghepBgorBgEEAYI3AwMBMYIXmTCCF5UGCSqGSIb3DQEHAqCCF4YwgheC
# AgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFaBgsqhkiG9w0BCRABBKCCAUkEggFFMIIB
# QQIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCAkus7dh5upOm58/8zj
# KOPBK4CGU57uGTUa6NjfgpPpfwIGaev4hMCHGBMyMDI2MDUwMzE0MzEzMC44ODJa
...
# BDAvBgkqhkiG9w0BCQQxIgQgnLUSd7n8MewNyfzOZyXn0BVpIzXWi6Q/9ZbODHOA
# PDwwgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9BCCxtpXMXEiLJzrqM77ep4rT
...
# j4F1dbuZEiYSbsNkqQniMA0GCSqGSIb3DQEBCwUABIICAEJtPlstt4FdxFY7a+mf
# uNnGqDnBWWOiWTBuu5N5ZlOgvzvEaYXNcXZob9TWZbKZufagow9xnStLa4bm4IS5
# 6OawZF8VrqtzGLO+QBdA3ILAaJBUWvxVZ+lYnGffWq5No1J7TwqRjATZoDPnlL9a
# BzXMJupeFqaxxP+nqRCO1cmF01ME0KSiN2x7x+d+jwRId2pT8gyA5FlZRJBywiaL
# nBf0g1qBxitpTMcufbgTRqs0mHGHQi6r8ddt07T9WSAXRyWSz5m0S+lX6YvP5mok
# ibrMQVlCEb6XQMT82Qb3xIo5MlIzcVgKA/Y8SxrrhSsTDtZfIVjHNUTwPjvlS8Uw
# 4b7ac/gAgs2dO/QqnmRHKR6A+a27r+QfrigBJVdv1b5B2NffXSMpHBGsrPMc8aAr
# MnHWBv6Bx91B5OGYiHpI17zoldkUk1qPA51ESTZVgHFn+78rVu+SYlW9PIZc2Lua
# i6lS47iPUpo9E6R/aTsjAFaioiZC5YCbA4Kq1ZwiZONqtpbEey2Cn7TikC2563x+
# PRpR+gFerZiFvh83rARYCfuC6e3uj3eqXlQI5yBCSabHoldg+6p/vYs+cD3wwRm6
# MsvKcZL54jtBhJoTyBLvDuet0aUOg5NAd9Zl4Uo8fnx7J5K4iVEtUWb3AvVNFhre
# 4IAp3W/ho4rcylb/iirhBEC8
