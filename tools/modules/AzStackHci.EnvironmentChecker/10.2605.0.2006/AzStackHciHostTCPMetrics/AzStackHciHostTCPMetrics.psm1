$MetaData = @{
    ...
    "UIName" = 'Azure Stack HCI Host TCP Metrics'
    "UIDescription" = 'Check host-to-host TCP network connectivity and performance.'
}

function Test-AzStackHciHostTCPMetrics
{
    <#
    .SYNOPSIS
        Wrapper function to invoke Host TCP Metrics validation during deployment.
    .DESCRIPTION
        This function serves as a wrapper for the Invoke-AzStackHciHostTCPMetricsValidation tool.
        It is designed to be called by the deployment framework, gathers the necessary parameters
        like PSSessions from the environment, and executes the validation. This test is specifically
        scoped to run only during the 'Deployment' operation.
    .PARAMETER Parameters
        A hashtable containing all the necessary parameters for the environment,
        including host information to establish PSSessions.
    .PARAMETER OperationType
        The lifecycle operation type. This validator will only run if the type is 'Deployment'.
    .PARAMETER FailFast
        A switch to indicate if the validation process should stop on the first failure.
    #>
    [CmdletBinding()]
    param (
    ...
    try
    {
        ...
        # Disable module auto-loading and explicitly import modules as needed.
        ...
        # Import dependent modules using the environment validator helper
        [EnvironmentValidator]::EnvironmentValidatorImport($Parameters)
        $ENV:EnvChkrOp = $OperationType

        Trace-Execution "Starting Host TCP Metrics validation"
        # This validator is only intended for the Deployment scenario.
        switch ($OperationType)
        {
            'Deployment'
            {
                # Establish PSSessions to all hosts in the deployment
                $psSession = [EnvironmentValidator]::NewPsSessionAllHosts($Parameters)

                # Prepare parameters to call the validation logic function
                $param = @{
                    PSSession   = $psSession
                    PassThru    = $true
                ...
                Trace-Execution "Invoking Invoke-AzStackHciHostTCPMetricsValidation"

                # Execute the validation and store the results
                [array]$Result = AzStackHci.EnvironmentChecker\Invoke-AzStackHciHostTCPMetricsValidation @param

                # Parse the raw results into the standardized format for the framework
                ...
                if ([EnvironmentValidator]::ParseResult.OverloadDefinitions -match 'EceInterfaceParameters')
                {
                    ...
                    return [EnvironmentValidator]::ParseResult($Result, 'HostTCPMetrics', $FailFast, $Parameters)
                }
                else
                {
                    Trace-Execution "ParseResult method does not support EceInterfaceParameters argument"
                    return [EnvironmentValidator]::ParseResult($Result, 'HostTCPMetrics', $FailFast)
                }
                #endregion
            }
            default
            {
                # This block should not be reached due to the Parameter's ValidateSet, but is included for safety.
                Trace-Execution "Host TCP Metrics validation is not applicable for OperationType '$($OperationType)'. Skipping."
            }
        }
    }
    catch
    {
        Trace-Execution "Host TCP Metrics Validator failed. Error: $_"
        Trace-Execution "StackTrace: $($_.ScriptStackTrace)"
        throw $_
    }
    finally
    {
        # Restore the original module auto-loading preference
        if ($backupPSModuleAutoLoadingPreference)
        {
            $PSModuleAutoLoadingPreference = $backupPSModuleAutoLoadingPreference
        }

        # Clean up and remove the PSSessions
        if ($psSession)
        {
            $psSession | Microsoft.PowerShell.Core\Remove-PSSession -ErrorAction SilentlyContinue
            Trace-Execution "Cleaned up PSSessions for Host TCP Metrics validation."
        }
    }
}

Export-ModuleMember -Function Test-AzStackHciHostTCPMetrics -Variable MetaData
# SIG # Begin signature block
# MIIncQYJKoZIhvcNAQcCoIInYjCCJ14CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC