##------------------------------------------------------------------
##  <copyright file="ReportStatus.ps1" company="Microsoft">
...
<#
.SYNOPSIS
    Generate json file for recording current status

.EXAMPLE
    PS C:\>ReportStatus.ps1 -Name $Name `
                            -Operation $Operation `
                            -Status $Status `
                            -Code $Code `
                            -Message $Message

    Generate a json file with transitioning status

.OUTPUTS
    Sample output of json status (currently not entire status is outputted as of now)
...
try {
    ...
    $status_report = [ordered]@{}
    $status_report.version = "1.0"
    $status_report.timestampUTC = [System.DateTime]::UtcNow.ToString("s") + "Z" 
    $status_report.status = [ordered]@{}
    $status_report.status.name = $Name
    $status_report.status.operation = $Operation
    $status_report.status.status = $Status
    $status_report.status.code = $Code
    $status_report.status.formattedMessage = [ordered]@{}
    $status_report.status.formattedMessage.lang = "en-US"
    $status_report.status.formattedMessage.message = $Message
    $status_report.status.substatus = $null

    $statusList += $status_report
    $statusJson = ConvertTo-Json $statusList -Depth 10
    $retryCount = 1
    $isStatusWritten = $false
    while (-not (Test-Path $statusFilePath) -or `
            (-not $isStatusWritten -and $retryCount -le 3)) {

        Write-Log "[$functionName] Attempt #$retryCount : Writing to status file at $statusFilePath"

        Out-File `
            -FilePath $statusFilePath `
            -InputObject $statusJson `
            -Encoding utf8 `
            -ErrorAction SilentlyContinue `
            -ErrorVariable WriteError `
            -Verbose:$false `
            | Out-Null

        if ($WriteError) {
            Write-Log "[$functionName] Writing to status file failed with error: $WriteError" -Level "ERROR"
    ...
    if ($isStatusWritten) {
        Write-Log "[$functionName] Exiting. Successfully wrote to status file."
    }
    else {
        Write-Log "[$functionName] Exiting. Failed to write to status file after $retryCount attempts." -Level "ERROR"
    }
}
catch {
    $customErrorMessage = "[$functionName] Failed to write to status file, "
    ...
    Write-Log "[$functionName] StatusFilePath - '$statusFilePath', Contents - ($statusJson). Exception details - $exceptionMessage" -Level "ERROR"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCXv+EvjVQwCPo0
# IhZ3rik0NcTVMyz1WoSwYNjCt1FQ1aCCDLowggX1MIID3aADAgECAhMzAAACHU0Z
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIKboDm/1
# G5Zt9Odb0ZtMUmTjUCdimf32h8toBw8JdrAWMEIGCisGAQQBgjcCAQwxNDAyoBSA
# EgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5jb20w
# DQYJKoZIhvcNAQEBBQAEgg