...
Import-Module $PSScriptRoot\AzStackHci.Bootstrap.Helpers.psm1 -DisableNameChecking -Global

function Invoke-AzStackHciBootstrapValidation
{
    <#
    .SYNOPSIS
        Perform AzStackHci Bootstrap Validation
    .DESCRIPTION
        Perform AzStackHci Bootstrap Validation
    .EXAMPLE
        PS C:\> Invoke-AzStackHciBootstrapValidation
        Perform all Bootstrap validations against localhost.
    .PARAMETER PassThru
    ...
    try
    {
        if (!$PSBoundParameters.ContainsKey('HardwareClass'))
        {
            $PSBoundParameters.HardwareClass = "Small"
        }

        if ([string]::IsNullOrEmpty($ENV:EnvChkrOp))
        {
            $ENV:EnvChkrOp = 'Bootstrap'
        }
        ...
        $Results = @()
		$hardwareTests = @('Test-Processor', 'Test-NetAdapter', 'Test-MemoryCapacity',
						   'Test-MemoryProperties', 'Test-Gpu', 'Test-Baseboard', 'Test-Model', 'Test-VirtualizationBasedSecurity',
						   'Test-PhysicalDisk', 'Test-TpmVersion', 'Test-TpmProperties', 'Test-SecureBoot', 'Test-MinCoreCount')
		$softwareTests = @('Test-NtpServer')
        $OSImageRecipeValidationTests = @('Test-BaseOSimage', 'Test-FeaturesOnDemand', 'Test-InstalledAdditionalFiles',
                                          'Test-InstalledPackages', 'Test-LatestCumulativeUpdate', 'Test-BuildId',
                                          'Test-SolutionVersion')
		$connectivityTests = @()

		if($hardwareTests.Count -gt 0)
		{
			$Params = @{
				Include = $hardwareTests
                OutputPath = $OutputPath
                PassThru = $true
                HardwareClass = $HardwareClass
                ClusterPattern = $ClusterPattern
            }
			$Results += Invoke-AzStackHciHardwareValidation @Params
		}
		if($OSImageRecipeValidationTests.Count -gt 0)
		{
			$Params = @{
				Include = $OSImageRecipeValidationTests
                OutputPath = $OutputPath
                PassThru = $true
            }
			$Results += Invoke-AzStackHciOSImageRecipeValidation @Params
		}
		if($softwareTests.Count -gt 0)
		{
			$Params = @{
				Include = $softwareTests
                OutputPath = $OutputPath
                PassThru = $true
            }
			$Results += Invoke-AzStackHciSoftwareValidation @Params
		}
		if($connectivityTests.Count -gt 0)
		{
			$Params = @{
				Include = $connectivityTests
				Exclude = (Get-AzStackHciConnectivityServiceName | Where-Object {$_ -ne 'MOC Stack'})
                OutputPath = $OutputPath
                PassThru = $true
            }
			# Connectivity has a mandatory set of services to validate, so if you want to me more specific, you need to exclude everything but your desired services (in my example MOC Stack)
			# It also has a set of proxy, tls inspection and dns tests, I don't know if there's are required or applicable for bootstrap, we would have to explicitly exclude them from the results.
			#$Results += Invoke-AzStackHciConnectivityValidation $Params
		}

        # Feedback results - user scenario
        if (-not $PassThru)
        {
            Write-Host 'Bootstrap Results'
            Write-AzStackHciResult -Title 'Bootstrap' -Result $Results
            Write-Summary -Result $Results -Property1 Detail
        }
        else
        {
            return $Results
    ...
    finally
    {
        ...
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 'Bootstrap' -Value $Results -Force
...
function Invoke-AzStackHciBootstrapHardwarePrerequisitesValidation
{
    <#
    .SYNOPSIS
        Perform AzStackHci Bootstrap Hardware PreRequisites Validation
    .DESCRIPTION
        Perform AzStackHci Bootstrap Hardware PreRequisites Validation
    .EXAMPLE
        PS C:\> Invoke-AzStackHciBootstrapHardwarePrerequisitesValidation
        Perform all Bootstrap Hardware PreRequisites validations against localhost.
    .PARAMETER PassThru
        Return PSObject result.
    .PARAMETER HardwareClass
        Hardware class: Small, Medium, or Large.
    ...
    try
    {
        ...
        # Run other validators
        $Results = @()
        $hardwareTests = @('Test-TpmVersion', 'Test-TpmProperties', 'Test-SecureBoot', 'Test-MemoryCapacity', 'Test-MinCoreCount')
        ...
        if (-not $PassThru)
        {
            Write-Host 'Bootstrap Hardware PreRequisites Results'
            Write-AzStackHciResult -Title 'BootstrapHardwarePreRequisites' -Result $Results
            Write-Summary -Result $Results -Property1 Detail
        }
        else
        {
    ...
    finally
    {
        ...
        # Write validation result to report object and close out report
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 