...
$MetaData = @{
    ...
    "UIName" = 'Azure Stack HCI OSImageRecipe'
    "UIDescription" = 'Check Operating System requirements'
}
function Test-AzStackHciOSImageRecipe
{
    param (
    ...
    try
    {
        ...
        if ($OperationType -eq 'Deployment')
        {
            ...
            $Params = @{
                ...
                PassThru = $true
                Exclude = @('Test-ComposedBuildVersion')
            }

            Trace-Execution "Starting OSImageRecipe validation for Deployment, detail output can be found in $($outputPath)\AzStackHciEnvironmentChecker*"
            $OSImageRecipeResult = AzStackHci.EnvironmentChecker\Invoke-AzStackHciOSImageRecipeValidation @Params
        }
        elseif ($OperationType -eq 'AddNode')
        {
            ...
            $params = @{
                ...
                ClusterPattern = $ClusterPattern
                PassThru = $true
                Include = @('Test-ComposedBuildVersion')
            }
            Trace-Execution "Starting OSImageRecipe validation for AddNode, detail output can be found in $($outputPath)\AzStackHciEnvironmentChecker*"
            $OSImageRecipeResult = AzStackHci.EnvironmentChecker\Invoke-AzStackHciOSImageRecipeValidation @Params

            # TO DO REMOVE THIS BLOCK AFTER CI ENVIRONMENT IS FIXED
            # If this is a CI environment unblock nodeAdd and repairNode CI runs for now,
            # we must fix these CI pipelines to pre-deploy the the base cluster version on all nodes
            # this way the node being used to add will be running the same version as the cluster.
            if (Get-ItemProperty -Path HKLM:\SOFTWARE\Microsoft\SQMClient -Name IsCIEnv -ErrorAction SilentlyContinue)
            {
                Trace-Execution "CI Environment. Suppress nodeAdd failures if the new node is not running the same version as the cluster."
                $OSImageRecipeResult | Where-Object Name -eq 'AzStackHci_OSImageRecipeValidation_NodeAdd_Build_ID' | ForEach-Object {
                    $_.Severity = 'WARNING'
        ...
        if ([EnvironmentValidator]::ParseResult.OverloadDefinitions -match 'EceInterfaceParameters')
        {
            ...
            return [EnvironmentValidator]::ParseResult($OSImageRecipeResult, 'OSImageRecipe', $FailFast, $Parameters)
        }
        else
        {
            Trace-Execution "ParseResult method does not support EceInterfaceParameters argument"
            return [EnvironmentValidator]::ParseResult($OSImageRecipeResult, 'OSImageRecipe', $FailFast)
        }
    }
    catch
    {
...
Export-ModuleMember -Function Test-AzStackHciOSImageRecipe -Variable MetaData
# SIG # Begin signature block
# MIInSQYJKoZIhvcNAQcCoIInOjCCJzYCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD2glwoGprEPt+h
# BlZ0QRlkweOcSLOP1gnTIRIWPwpvl6CCDLowggX1MIID3aADAgECAhMzAAACHU0Z
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIBDpnkME
# 4dRNdDAqfMglhgeDGXTnHGie4Ll8SW8WgOLlMEIGCisGAQQBgjcCAQwxNDAyoBSA
# EgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5jb20w
# DQYJKoZIhvcNAQEBBQAEgg