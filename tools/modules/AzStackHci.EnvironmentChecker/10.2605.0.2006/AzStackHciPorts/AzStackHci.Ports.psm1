...
Import-Module $PSScriptRoot\AzStackHci.Ports.Helpers.psm1 -DisableNameChecking -Global

function Invoke-AzStackHciPortValidation
{
    <#
    .SYNOPSIS
        Perform AzStackHci Port Validation
    .DESCRIPTION
        Perform AzStackHci Port Validation
    .EXAMPLE
        PS C:\> Invoke-AzStackHciPortValidation
        Perform port validation against all built in service targets from localhost.
    .EXAMPLE
        PS C:\> $Credential = Get-Credential -Message "Credential for local cluster"
        PS C:\> Invoke-AzStackHciPortValidation -Credential $Credential
        Perform port validation against all built in service targets from all cluster nodes of local system.
    .EXAMPLE
        PS C:\> Invoke-AzStackHciPortValidation -Service ARC
        Perform port validation against ARC targets from localhost.
    .EXAMPLE
        ...
        PS C:\> Invoke-AzStackHciPortValidation -PsSession $RemoteSystemSession
        Perform port validation against all built in service target from pre-existing remote PS session.
    .EXAMPLE
        PS C:\> $RemoteSystem = 'node01.contoso.com'
        PS C:\> $Credential = Get-Credential -Message "Credential for $RemoteSystem"
        PS C:\> Invoke-AzStackHciPortValidation -ComputerName $RemoteSystem -Credential $Credential
        Perform port validation against all built in service target from a remote system.
    .EXAMPLE
        PS C:\> Invoke-AzStackHciPortValidation -CustomTargetFile C:\CustomPortRules\Custom.Json
        Perform port validation against rules/ports defined in C:\CustomPortRules\Custom.Json
    .EXAMPLE
        PS C:\> Invoke-AzStackHciPortValidation -CustomTargetFile C:\CustomPortRules\Custom.Json -PassThru
        Perform port validation against rules/ports defined in C:\CustomPortRules\Custom.Json, returning PSObject to pipeline.
    .INPUTS
        Inputs (if any)
    .OUTPUTS
        Output (if any)
    ...
    param (
        ...
        [Parameter(Mandatory = $false, HelpMessage = "Specify the services to target for port validation.")]
        [ArgumentCompleter({ Get-AzStackHciPortServiceName })]
        [ValidateScript({ $_ -in (Get-AzStackHciPortServiceName) })]
        [string[]]
        $Service,

        [Parameter(Mandatory = $false, HelpMessage = "Specify the Operation Type to target for port validation. e.g. Deployment, Update, etc...")]
        [ArgumentCompleter({ Get-AzStackHciPortOperationName })]
        [ValidateScript({ $_ -in (Get-AzStackHciPortOperationName) })]
        [string[]]
        $OperationType,

        [Parameter(Mandatory = $false, HelpMessage = "Supply a path to a custom json for validation.")]
        [string[]]
        $CustomTargetFile,
    ...
    try
    {
        ...
        # Create neccessary PsSessions
        if (-not $PsSession)
        {
            $PsSession = Open-PsSession -ComputerName $ComputerName -Cluster $Cluster -Authentication $Authentication -Credential $Credential
        }

        # TO DO: Omit system?
        $PortTargets = Get-AzStackHciPortTarget -Service $Service -OperationType $OperationType -FilePath $CustomTargetFile
        ...
        if ($PsSession)
        {
            foreach ($Session in $PsSession)
            {
                if ($Session.State -ne 'Opened')
                {
                    ...
                    {
                        Log-Info ($lcTxt.RDPFail -f $Session.ComputerName, $_.Exception.Message) -type Warning -ConsoleOut
                        break
                    }
                }
                foreach ($target in $PortTargets)
                {
                    $PortResult += Invoke-PortConnection -Target $Target -PsSession $Session
                }
            }
        }
        else
        ...
        if (-not $PassThru)
        {
            Write-Host 'Port Validation Results'
            $PortResult.service | Sort-Object | Get-Unique | Where-Object { $PSITEM -ne 'System' } | ForEach-Object {
                Write-AzStackHciResult -Title $PSITEM -Result ($PortResult | Where-Object Service -EQ $PSITEM)
            }
        }
        else
        {
        ...
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 