...
Import-Module $PSScriptRoot\AzStackHci.MOCStack.Helpers.psm1 -DisableNameChecking -Global
Import-Module $PSScriptRoot\..\AzStackHciConnectivity\AzStackHci.Connectivity.Helpers.psm1 -DisableNameChecking -Global

function Invoke-AzStackHciMOCStackValidation
{
    <#
    .SYNOPSIS
        Perform AzStackHci MOCStack Validation
    .DESCRIPTION
        Perform AzStackHci MOCStack Validation
    .EXAMPLE
        PS C:\> Invoke-AzStackHciMOCStackValidation
        Perform all MOCStack validations against localhost.
    .EXAMPLE
        ...
        PS C:\> Invoke-AzStackHciMOCStackValidation -PsSession $RemoteSystemSession
        Perform all MOCStack validations pre-existing remote PS sessions.
    .PARAMETER PsSession
        Specify the PsSession(s) used to validation from.
	.PARAMETER OperationType
        Specify the Operation Type to target for MOC validation. e.g. Deployment, Update, etc
    .PARAMETER PassThru
    ...
    param (
		...
		[Parameter(Mandatory = $false, HelpMessage = "Specify the Operation Type to target for MOC validation. e.g. Deployment, Update, etc...")]
        [ValidateSet('Deployment', 'Upgrade', 'PreUpdate', 'AddNode')]
        $OperationType,

		[Parameter(Mandatory = $false, HelpMessage = "Tests to include.")]
        [ArgumentCompleter({ Get-TestListByFunction -ModuleName AzStackHci.MOCStack.Helpers })]
        [ValidateScript({ $_ -in (Get-TestListByFunction -ModuleName AzStackHci.MOCStack.Helpers) })]
        [string[]]
        $Include,
    ...
    try
    {
        ...
        if (Test-Elevation)
        {
            Log-Info -Message ($VvsTxt.ElevationModeInfo) -Type Info
        }
        else
        {
            Log-Info -Message ($VvsTxt.ElevationModeMsg) -Type Error -ConsoleOut
            throw $($VvsTxt.ElevationModeErrMsg)
        }
        ...
        $Exclude += @('Test-MOCStackNetworkPort')
		$deploymentTestListToExclude = @('Test-MOCStackClusterNode','Test-MOCStackCloudAgent','Test-MOCStackNodeAgents')
        $upgradeTestListToExclude = @('Test-MOCStackCloudAgent','Test-MOCStackNodeAgents')

        if ($RegionName -eq 'AzureLocal') {
            $deploymentTestListToExclude += @('Test-MOCStackNetworkPort')
        }

		# Get the list of unsupport test based on operationType
		if ($OperationType -eq 'Deployment' -or $OperationType -eq 'AddNode')
		{
			$Exclude += $deploymentTestListToExclude
		}
		...
		$testList = Get-TestListByFunction -ModuleName AzStackHci.MOCStack.Helpers
		...
		$instanceCount = 0
		$MOCStackResult = @()
		$progressActivity = "Validating AzStackHci MOCStack Component"
		...
		if ($PsSession)
        {
            foreach ($Session in $PsSession)
            {
                if ($Session.State -ne 'Opened')
                {
                    ...
                    {
                        $psSessionFail = $VvsTxt.PsSessionFail -f $Session.ComputerName, $_.Exception.Message
			...
			{
				$outputMsg = $VvsTxt.TestProgessInfo -f $test, ($PsSession.ComputerName -join ',')
                ...
                # Add RegionName if the test supports it AND RegionName was provided
                if ((Get-Command $test).Parameters.ContainsKey('RegionName') -and $RegionName)
                {
                    $splat.RegionName = $RegionName
                }

                $MOCStackResult += Invoke-Expression "$test @splat"
			}
		}
        ...
        if (-not $PassThru)
        {
            Write-Host 'MOCStack Results'
            Write-AzStackHciResult -Title 'MOCStack' -Result $MOCStackResult
            Write-Summary -Result $MOCStackResult -Property1 Detail
        }
        else
        {
            return $MOCStackResult
        }
    }
    catch
    {
    ...
    finally
    {
        ...
        foreach ($result in $MOCStackResult)
        {
            Write-ETWResult -Result $result
        }
        # Write validation result to report object and close out report
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 