...
Import-Module $PSScriptRoot\AzStackHci.RemoteManagement.Helpers.psm1 -DisableNameChecking -Global

function Invoke-AzStackHciRemoteManagementValidation
{
    <#
    .SYNOPSIS
        Perform Remote Management Validation
    .DESCRIPTION
        Performance Remote Management validation to ensure nodes management technologies are open and available for remote querying.
    .EXAMPLE
        Invoke-AzStackHciRemoteManagementValidation -ComputerName Node01, Node02 -Credential $Credential
    .PARAMETER ComputerName
        Specify the ComputerName(s) used to validation from.
    .PARAMETER Credential
        Specify the Credential to use for the validation.
    .PARAMETER PassThru
    ...
    [CmdletBinding()]
    param (
        # VALIDATOR OWNER TO DO: Add any specific parameters needed to run the validation, e.g. NTPServer name, ARMToken etc..

        # VALIDATOR OWNER TO DO: decide if you want to run this test against a remote system or locally
        [Parameter(Mandatory = $true, HelpMessage = "ComputerName(s) to test")]
        [string[]]
        $ComputerName,

        [Parameter(Mandatory = $true, HelpMessage = "Credential to use when testing ComputerName(s).")]
        [pscredential]
        $Credential,

        # VALIDATOR OWNER TO DO: Leave the parameters below as is, they are used to control the test execution and provide consistent experience
        [Parameter(Mandatory = $false, HelpMessage = "Tests to include.")]
        [ArgumentCompleter({ Get-TestListByFunction -ModuleName AzStackHci.RemoteManagement.Helpers })]
        [ValidateScript({ $_ -in (Get-TestListByFunction -ModuleName AzStackHci.RemoteManagement.Helpers) })]
        [string[]]
        $Include,
    ...
    try
    {
        ...
        $testList = Get-TestListByFunction -ModuleName AzStackHci.RemoteManagement.Helpers
        # VALIDATOR OWNER TO DO: This code auotmatically selects the tests to run based on the include/exclude parameters,
        # VALIDATOR OWNER TO DO: if you require more granular control, such as ordering or grouping tests, you can modify this code
        ...
        $i = 0
        $Result = @()
        $ProgressActivity = "Checking AzStackHci RemoteManagement Compatibility"
        if ($PsSession)
        {
            ...
            :noTestsBreak foreach ($test in $script:envchktestList)
            {
                ...
                $Result += Start-ValidatorTest -TestName $test -Parameters $invokeParameters -ValidatorName 'RemoteManagement' -EnableTelemetry
            }
        }
        else
        {
        ...
        if (-not $PassThru)
        {
            Write-Host 'RemoteManagement Results'
            Write-AzStackHciResult -Title 'RemoteManagement' -Result $Result
    ...
    finally
    {
        ...
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 'RemoteManagement' -Value $Result -Force
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCus1eEdc+ky/Tc
# OvSqMXLaez7IFsCqQv8Q1ojz0pZkPqCCDMkwggYEMIID7KADAgECAhMzAAACHPrN
...
# hvcNAQkEMSIEINwfSTPAs09Xn0qNw7bxoYVGJiZ822LAerc2lzZ7znMVMEIGCisG
# AQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3
# Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQ