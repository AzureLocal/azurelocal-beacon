...
$MetaData = @{
    ...
    "UIName" = 'Azure Stack HCI Cluster'
    "UIDescription" = 'Check Cluster requirements'
}
function Test-AzStackHciCluster
{
    param (
    ...
    try
    {
        ...
        {
            $Params += @{
                Exclude = @('Test-ClusterPreReq')
                OperationType = $OperationType
            }
        }
        elseif (($OperationType -eq 'Deployment') -or ($OperationType -eq 'PreUpdate'))
        {
            # if bootstrap has no external connectivity FailoverClusters module will not be available.
            # if FailoverClusters module is not available, try and install it again.
            if (-not (Get-WindowsFeature -Name "RSAT-Clustering-PowerShell").Installed)
            {
                Trace-Execution "Installing RSAT-Clustering-PowerShell..."
                Install-WindowsFeature -Name "RSAT-Clustering-PowerShell"
                Trace-Execution "RSAT-Clustering-PowerShell installed."
            }

            $PsSession = [EnvironmentValidator]::NewPsSessionAllHosts($Parameters)

            # Get fqdns of nodes to set in TrustedHosts
            $domainName = $Parameters.Roles.Domain.PublicConfiguration.PublicInfo.DomainConfiguration.Fqdn
            $hostIpMapping = Get-NetworkMgmtIPv4FromECEForAllHosts -Parameters $Parameters
            $clusterHostIPs = $hostIpMapping.Values -join ','
            $clusterHostNames = $hostIpMapping.Keys -join ','
            $fqdns = ($hostIpMapping.Keys | ForEach-Object {"$_.$domainName"}) -join ','
            $clusterHosts = "$clusterHostIPs,$clusterHostNames,$fqdns"

            # If LocalIdentity add node.local to trustedhosts as well.
            $cloudRole = $Parameters.Roles["Cloud"].PublicConfiguration
            if ($cloudRole.PublicInfo.DeployADLess -eq 'true')
            {
                $localIdentityFqdn = ($hostIpMapping.Keys | ForEach-Object {"$_.local"}) -join ','
                $clusterHosts = "$clusterHosts,$localIdentityFqdn"
            }

            Trace-Execution "Hosts to add to TrustedHosts: $clusterHosts"
            $clusterHosts -split ',' | Foreach-Object { [EnvironmentValidator]::SetTrustedHosts($PsItem) }
            ...
            # Pass StorageType so Test-Cluster can exclude SAN-incompatible disk tests
            try {
                $storageType = $Parameters.Roles["Storage"].PublicConfiguration.PublicInfo.StorageConfiguration.Type
                if (-not [string]::IsNullOrEmpty($storageType)) {
                    Trace-Execution "StorageType detected: $storageType"
                    $Params['StorageType'] = $storageType
                }
            }
            catch {
                Trace-Execution "StorageType not available in parameters, using default (S2D)"
            }
        }
        else
        {
        ...
        $ClusterResult = AzStackHci.EnvironmentChecker\Invoke-AzStackHciClusterValidation @Params

        # "PreUpdate" is newly added, the results should be INFORMATIONAL only for now.
        # NOTE: PreUpdate is currently disabled in role.xml.
        # Pending Test-Cluster PKU2U compatibility.
        if ($OperationType -eq 'PreUpdate') {
            $ClusterResult | ForEach-Object { if ($_.Severity -ine "INFORMATIONAL") { $_.Severity = "INFORMATIONAL" } }
        }
        ...
        if ([EnvironmentValidator]::ParseResult.OverloadDefinitions -match 'EceInterfaceParameters')
        {
            ...
            return [EnvironmentValidator]::ParseResult($ClusterResult, 'Cluster', $FailFast, $Parameters)
        }
        else
        {
            Trace-Execution "ParseResult method does not support EceInterfaceParameters argument"
            return [EnvironmentValidator]::ParseResult($ClusterResult, 'Cluster', $FailFast)
        }
    }
    catch
    {
    ...
    finally
    {
        ...
        if ($localIdentityFqdn)
        {
            # Get trusted Hosts
            $trustedHosts = (Get-Item WSMan:\localhost\Client\TrustedHosts).Value -split ','
            Trace-Execution "TrustedHosts after cluster validation: $trustedHosts"
            $trustedHosts = $trustedHosts | Where-Object {  $_ -notin ($localIdentityFqdn -split ',') }
            Set-Item WSMan:\localhost\Client\TrustedHosts -Value ($trustedHosts -join ',') -Force
            Trace-Execution "Removed $localIdentityFqdn from TrustedHosts."
            Trace-Execution ("TrustedHosts after removal: {0}" -f (Get-Item WSMan:\localhost\Client\TrustedHosts).Value)
        }
    }
}

Export-ModuleMember -Function Test-AzStackHciCluster -Variable MetaData
# SIG # Begin signature block
# MIInRgYJKoZIhvcNAQcCoIInNzCCJzMCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAByT5fNY7pbqx9
# f9666c4PaG10rN/ktYEajq72jssptaCCDLowggX1MIID3aADAgECAhMzAAACHU0Z
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEICjKUzy6
# ZU4Rp5E44UnQCh/8Xl7dW6LI2iZMdHLxDYHMMEIGCisGAQQBgjcCAQwxNDAyoBSA
# EgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3Lm1pY3Jvc29mdC5jb20w
# DQYJKoZIhvcNAQEBBQAEggEAVa82RMNd6rCuOoiPHd6yWHEu/VyZjJOm+zNYEzcv
# qZMCT3+HiL6Dk62NkKd6ymxyc9n3ZzwIT/G4TYCE6zgFsZUeZTqTt05hHI61cKh1
# K8aVPHHwVASNIcx+b6ZyVn2JBrorkSKhpaSpDQjd77udbS56bL1p3H+UKgST2hpm
# zQcF09wN3Cd3OVHZ6xWlsGL4gwC45pJabaQvC/Z9jOWoQsxiPmAh7rToDunzSvUw
# 1e8PWDsCPOltchTA2MElFBSkD3ZGILA6Qij/0hIVLatRvE97fiLqoxqIMRXuLlIh
# cbdZ3FsMnRpR7qujEKy1sa/eZFlnS+j28k6oorxPRlqiWKGCF5QwgheQBgorBgEE
# AYI3AwMBMYIXgDCCF3wGCSqGSIb3DQEHAqCCF20wghdpAgEDMQ8wDQYJYIZIAWUD
# BAIBBQAwggFSBgsqhkiG9w0BCRABBKCCAUEEggE9MIIBOQIBAQYKKwYBBAGEWQoD
# ATAxMA0GCWCGSAFlAwQCAQUABCCN/HyONLdiyMpK1AVeLEidF9sG0jj+xrP4wyME
# 8B3N+gIGaedeW7IXGBMyMDI2MDUwMzE0MzExMC4xNzJaMASAAgH0oIHRpIHOMIHL
...
# SIb3DQEJEAEEMC8GCSqGSIb3DQEJBDEiBCAbOcp279SwAppJEYGpVmWHEg90GOpE
# BjhLxtOtnGzJWDCB+gYLKoZIhvcNAQkQAi8xgeowgecwgeQwgb0EIFWxikZRYGNf
...
# DcPwUXJi0s7Mn7qi2o2Z1BZ5LZlH28swDQYJKoZIhvcNAQELBQAEggIASuoNcfkz
# i+FHeacfdVaB/IxCP143IcQkEoogOCnOWUss/lZVFQWUZRpg1ReFC0GY+zKKhjnn
# CpaNiP5qAkFmzy0A5BuKAr8a0MkTr5a0u0vx2Vw+UdU5L5EXlouPbE/8S/rfHOLo
# yUaRPmirkoTzGVBdrK/dyGxD4zf6RGKMpTg7NctTo3KwdIffRpez475/jtTZR6IE
# bl1qz1Ip48Qc9UQuVXek2jXMXo/ZJ7VnrjttP+d0IPhnvzwd8QuPXi/BUkQzwKMe
# 2srEXQ9m7OtXfdJg541jTK1Y/0MKGprD5G6vkFFJNVcbZgt89wz1jHHkHeLQf6Ux
# GeW4HOm7AVG+nc4dNzScMqclXmYO3MOGvBKTAI20a6ZwZz7gpLz1bR1RlTz5ZTi3
# Q6tc6dMYcbRl1Z5T+yr0EXUMnF9gSayIDVh1n0GfdW0d50/9ku+R6HNvyOkWxoSi
# ZyalGTXCCwE/Ord743W3tye+ClUsFqdAwmAX7bajaxblkaItd6E+7gDP3GplgKkJ
# nYlrQscDbc3CmU9Lf9zx4FP3jWX//6cAsXXzY1Yv7xNB0GTnJtVyD/mV6+eP4/61
# 1HCM1cfbnt6VemMEoZ4LMaAMYZTUTr16++Tt80KJnQYFBAeiybmCxaT50aK4Xw0n
# LNbNI/eKpKlw2W7vQ3s8s86+7g4hLLcOG/k=
