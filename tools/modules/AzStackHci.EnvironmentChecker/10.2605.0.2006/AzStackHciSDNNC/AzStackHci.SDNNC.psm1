...
Import-Module $PSScriptRoot\AzStackHci.SDNNC.Helpers.psm1 -DisableNameChecking -Global

function Invoke-AzStackHciSDNNCValidation
{
    <#
    .SYNOPSIS
        Perform AzStackHci SDN-NC Validation.
    .DESCRIPTION
        This function validates SDN-NC health.
    .EXAMPLE
        Invoke-AzStackHciSDNNCValidation -OutputPath "C:\Logs" -PassThru -CleanReport
    #>
    [CmdletBinding()]
    param(
    ...
    $ProgressActivity = "Validating AzStackHci SDN Network Controller health"

    try {
        $script:ErrorActionPreference = 'Stop'
        if ([string]::IsNullOrEmpty($OutputPath)) {
            $OutputPath = Join-Path -Path $env:TEMP -ChildPath "SDNNCValidation"
        }
        ...
        $allPossibleTests = @( "Test-NwkValidator_SDNNCDeployed", "Test-NwkValidator_SDNNCServices" )
        ...
        if (($script:envchktestList).Count -eq 0) {
            Log-Info "No SDN-NC tests selected to run."
            return
        }
        ...
        for ($i = 0; $i -lt ($script:envchktestList).Count; $i++) {
            ...
            $OpMsg = "Running SDN-NC test [{0}]" -f $test
        ...
        if (-not $PassThru) {
            Write-AzStackHciResult -Title "SDN-NC Validation Results" -Result $Result -ShowFailedOnly:$ShowFailedOnly -Seperator ': '
        ...
        $cmdletException = $_
        #throw $_ # Informational only, do not re-throw
    }
    finally {
        $Script:ErrorActionPreference = 'Continue'
        foreach ($r in $Result) { Write-ETWResult -Result $r }
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 'SDNNC' -Value $Result -Force
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDS1xTXEjpp2Msk
# MXOZjGElUzcjhPtay/4WzfWUK5Hl3KCCDMkwggYEMIID7KADAgECAhMzAAACHPrN
...
# hvcNAQkEMSIEIFayVY/tvEWfO9TmG196iWDWmYEyEBYNnWxS9D+RBmUgMEIGCisG
# AQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3
# Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEBBQAEggEAPIqQg8VymvKQbZUJWEWD
# Qj4xiqM/PCg2o3EpS1QlqDWR80URW13/rlMVaaxflFY0NvwO/DvNORArzmq5ikIc
# 9qI4Sd++Bn1EqTORBj+t1JTvZowGsMeKZWRvauafoFp7a7NY6O3YK/YtSLtJliY0
# LfyA7BlJZUYBMElw50Fdu9I2DoPmJF8AEHjiN2ZJTVvHri4tph46k1AL8Jd5UJs2
# XA7B35xpfeF+/MNyQDjpjVflxJ6tPrk0syFcK4aIamzULLtAt3//yboIANUS/nY9
# eCycO+AzaS8QpbbEyH1ClYmiikJ6fJKa31KLtiO+WruHJYL8aFBcUxSmfl5XlUWt
# xaGCF7AwghesBgorBgEEAYI3AwMBMYIXnDCCF5gGCSqGSIb3DQEHAqCCF4kwgheF
# AgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFaBgsqhkiG9w0BCRABBKCCAUkEggFFMIIB
# QQIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCDcFam1Ebtft+xQ5qoD
# t3WyM/zcp+fUisIHxX/+u+Z0lAIGaexvE19LGBMyMDI2MDUwMzE0MzExMC43NzFa
...
# CRABBDAvBgkqhkiG9w0BCQQxIgQgK1eVmT0Qb4RsMF5WOfBaaurZNOLLcXXOEft2
# RjEMkS8wgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9BCBwEPR2PDrTFLcrtQsK
...
# IFlR5RUBapAjgKe/JVm88ntNMA0GCSqGSIb3DQEBCwUABIICAH+46H6UuyLdkazT
# RRzmcM1xE+OttVL9l3nwxWcgsPFsGDOv1Ryg1AGsu68orNuaN2K9+9BZwu+XpSLH
# 5874J60AqK+1vxDKfTxZadWrj3NyulOhI91LkatHJr27RIb5hQ7qBPCKhPJJYVpM
# qzuLCDWOIX+Og6XObfPpxxAgLC+AqIo0PFbEBN5VKoDtJyNYH0GijpcFYxXIapf4
# Y5vMD+1WfTq5exLfh3klT4G/PWBpvRJzUh2Ho+WtyuHCVjJa5+08E45TglFXNOYq
# FdKHLf3QFZHc+DdDEAK9njVQIQYcyr4m9xcDYpXhSLGs3A0R46/hoGvW3RRhGtnM
# gGhhdQMOirT5P0kHQsCkFK33Edv/DOHEUeGQL2dtXe5RxvvvVxeU/QBg+vVfbICS
# 4N7CuBWEkNB6LB3P/TzEkox7iPkSdFrt+GLdMKGLowZu2Dgz3xGWAAAlQxyr1rbH
# VcJHaF1Gn3B/WACcNwwq2lA4DFHurYuC9o1ze0AH3vygcb3GoT3DhU2ERs76vi2e
# JfiFVyEMFV6Zb1wlka350B+zL/Q2Jr72s5MMdYedcckvhh9J5dsAjCDi9Pj+fZmu
# EjTlzinRLe9HYyWDlJ3RLXfkbI//wpq0inRnEyjfsOfNbhLFvmQOXVXEew9N7NGI
# oddrx8sykEP3KgujJbMTuyVjHrDS
