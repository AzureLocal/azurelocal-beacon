...
$MetaData = @{
    ...
    "UIName" = 'Azure Stack HCI Observability'
    "UIDescription" = 'Check Log Collection & Remote Support requirements'
}
function Test-AzStackHciObservability
{
    param (
    ...
    try
    {
        ...
        if ($OperationType -match 'Deployment|PreUpdate|Upgrade|Update|AddNode')
        {
			[array]$observabilityResult
            ...
            elseif ($OperationType -eq 'AddNode') {
                # This is only valid while the only test that runs is Observability volume size
                # if more tests are added this condition of only the AddNode may need to be revised
                # for add node, get PsSession to only the node being added
			...
			Trace-Execution "Starting Observability validation, detail output can be found in $($outputPath)\AzStackHciEnvironmentChecker*"
			if ($SkipLogCollectionValidation)
			{
				$observabilityResult = AzStackHci.EnvironmentChecker\Invoke-AzStackHciObservabilityValidation -PsSession $PsSession -OperationType $OperationType -SkipLogCollectionValidation -PassThru -OutputPath $outputPath -HardwareClass $HardwareClass
			}
			else
			{
				$observabilityResult = AzStackHci.EnvironmentChecker\Invoke-AzStackHciObservabilityValidation -PsSession $PsSession -OperationType $OperationType -PassThru -OutputPath $outputPath -HardwareClass $HardwareClass
			}

            # In CI supress observability volume exception
            if (Get-ItemProperty -Path HKLM:\SOFTWARE\Microsoft\SQMClient -Name IsCIEnv -ErrorAction SilentlyContinue)
            {
                Trace-Execution "CI Environment. Suppress observability volume"
                $observabilityResult | Where-Object Name -eq 'AzStackHci_Observability_Volume' | ForEach-Object {
                    $_.Severity = 'WARNING'
            ...
            if ([EnvironmentValidator]::ParseResult.OverloadDefinitions -match 'EceInterfaceParameters')
            {
                ...
                return [EnvironmentValidator]::ParseResult($observabilityResult, 'Observability', $FailFast, $Parameters)
            }
            else
            {
                Trace-Execution "ParseResult method does not support EceInterfaceParameters argument"
                return [EnvironmentValidator]::ParseResult($observabilityResult, 'Observability', $FailFast)
            }
        }
        else
        {
...
Export-ModuleMember -Function Test-AzStackHciObservability -Variable MetaData
# SIG # Begin signature block
# MIIncQYJKoZIhvcNAQcCoIInYjCCJ14CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDtvTdRpDUWw9sl
# hHCgSCgiBDnh65Xnn4C+twz188FhDKCCDMkwggYEMIID7KADAgECAhMzAAACHPrN
...
# hvcNAQkEMSIEIDmjYnaoI76KV0TKRtmcI8gECff4yquTpquKxUBsrk8WMEIGCisG
# AQQBgjcCAQwxNDAyoBSAEgBNAGkAYwByAG8AcwBvAGYAdKEagBhodHRwOi8vd3d3
# Lm1pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQ