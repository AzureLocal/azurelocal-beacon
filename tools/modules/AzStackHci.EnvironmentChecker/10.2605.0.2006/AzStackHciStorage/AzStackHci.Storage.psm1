...
Import-Module $PSScriptRoot\AzStackHci.Storage.Helpers.psm1 -DisableNameChecking -Global
function Invoke-AzStackHciStorageValidation
{
    <#
    .SYNOPSIS
        Perform AzStackHci Storage Validation
    .DESCRIPTION
        Perform AzStackHci Storage Validation to check the readiness for the storage subsystem on an exsting HCI stamp to be upgraded to the next generation of HCI stamp.
        The validation will cover the Storage Space Direct status, storage pool status, capacity for infrastructure volumes, etcs.
    .EXAMPLE
        PS C:\> Invoke-AzStackHciStorageValidation -PoolConfigXml $poolxml
    .INPUTS
        Inputs (if any)
    .OUTPUTS
        Output (if any)
    ...
    param (
        ...
        [ArgumentCompleter({ Get-TestListByFunction -ModuleName AzStackHci.Storage.Helpers })]
        [ValidateScript({ $_ -in (Get-TestListByFunction -ModuleName AzStackHci.Storage.Helpers) })]
        [string[]]
        $Include,
        ...
        [Parameter(Mandatory = $false, HelpMessage = "Resiliency setting name: Simple, Mirror, or Parity")]
        [ValidateSet("Simple", "Mirror", "Parity")]
        [String]$ResiliencySettingName,

        [Parameter(Mandatory = $false, HelpMessage = "Physical Disk Redundancy: 0, 1, 2")]
        [ValidateSet(0, 1, 2, 3)]
        ...
        [Parameter(Mandatory = $false, HelpMessage = "Operation type: Upgrade, or Deployment")]
        [ValidateSet('Upgrade','Deployment')]
    ...
    try
    {
        ...
        Write-Progress -Id 1 -Activity "Checking AzStackHci Dependancies" -Status "Storage Configuration" -PercentComplete 0 -ErrorAction SilentlyContinue

        if($OperationType -ne "Deployment")
        {
            # Get the clustered storage subsystem name (this acts like an assert)
            $storageSubSystem = Get-StorageSubSystem Cluster* -ErrorAction Ignore
            if (-not $storageSubSystem) {
                throw $lnTxt.ErrNoStorageSubsystemFound
            }
            Log-Info -Message "StorageSubSystem: `n$($storageSubSystem | ft | Out-String)"

            # Get cluster nodes
            $clusterNodes = Get-ClusterNode
            Log-Info -Message "Cluster Nodes: `n$($clusterNodes | ft | Out-String)"
            $nodeCount =  $clusterNodes.Count
        }

        # Run validations
        $TestList = Get-TestListByFunction -ModuleName AzStackHci.Storage.Helpers
        ...
        $progressParams = @{
            Id          = 1
            Activity    = "Checking AzStackHci Storage Configuration"
            Status      = "Testing '$storageSubSystem'"
        ...
        Log-Info "Storage Validation" -ConsoleOut:(-not $PassThru)
        if (-not $PassThru)
        {
            $progressParams = @{
                Id              = 3
                Activity        = "Formating Results"
                Status          = "Writing Results for '$storageSubSystem'"
                PercentComplete = 1
                ErrorAction     = 'SilentlyContinue'
            }
            Write-Progress @progressParams
            Write-AzStackHciResult -Title "${storageSubSystem}:" -Result $result -ShowFailedOnly:$ShowFailedOnly -Seperator ': '
    ...
    finally
    {
        ...
        $envcheckerReport | Add-Member -MemberType NoteProperty -Name 'Storage' -Value $result -Force
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD2Z31zcgJi+78G
# tQ9pOvpEm74+JHHYWRJlzZdwpawWfKCCDLowggX1MIID3aADAgECAhMzAAACHU0Z
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIE