<#############################################################
 ...
 $MetaData = @{
    ...
    "OperationType" =  @("PreAddNode","AddNode")
    "UIName" = 'Azure Stack HCI AddNode Check'
    "UIDescription" = 'Check AddNode inputs meet requirements'
}
function Test-AzStackHciAddNode
{
    param (
    ...
    try
    {
        ...
        # TO DO: Remove PreAddNode
        if ($OperationType -in "PreAddNode","AddNode")
        {
    		$runtimeParameters = $Parameters.RunInformation['RuntimeParameter']
            $NodeNames = $runtimeParameters['NodeName'] -split ' '
            [array]$AddNodeResult
            $Params = @{
                ComputerName = $NodeNames
                ActiveDirectoryCredential = [EnvironmentValidator]::GetDomainAdminCredential($Parameters)
            ...
            {
                # Exclude AD credential test if DeployADLess parameter is set to true
                Trace-Execution "Local Identity with AKV Deployment, skipping Test AD Credential."
                $Params.Exclude = @('Test-ADCredential')
            }

            # if repairnode, exclude Test-ClusterNodeName
            if ([string]::IsNullOrEmpty($runtimeParameters['IpAddress']))
            {
                $Params.Exclude += 'Test-ClusterNodeName'
            }

            # IpAddress is not a runtime parameter during repairnode
            # get from network helper instead
            # DHCP Case: Check to see if DHCP is enabled.
            # If DHCP is enabled, GetHostNicIPByName will returns host name and Ipv4OrHostName will be set to the HostName
            ...
            {
                [array]$Ips = $NodeNames | Foreach-Object {
                    [EnvironmentValidator]::GetHostNicIpByName($Parameters,$PsItem) | Select-Object -First 1
                }
                $Params += @{
                    Ipv4OrHostName = $Ips
                }
            ...
            {
                [array]$Ips = $runtimeParameters['IpAddress'] -split ' '
                $Params += @{
                    Ipv4OrHostName = $Ips
                }
            }

            Trace-Execution "Checking PSRemoting connectivity to $($Params.ComputerName)"
            $Params.Ipv4OrHostName | Foreach-Object { [EnvironmentValidator]::SetTrustedHosts($PsItem) }
            $AddNodeNode = [EnvironmentValidator]::GetNodeContext($Parameters)
            [array]$testSession = [EnvironmentValidator]::NewPsSessionByHost($Parameters, $AddNodeNode, $false)
            $testSession | Remove-PsSession

            Trace-Execution "Starting AddNode validation, detail output can be found in $($env:LocalRootFolderPath)\MASLogs\AzStackHciEnvironmentChecker*"
            $AddNodeResult = AzStackHci.EnvironmentChecker\Invoke-AzStackHciAddNodeValidation @Params

            # If this is a CI environment unblock/downgrade quorum result
            if (Get-ItemProperty -Path HKLM:\SOFTWARE\Microsoft\SQMClient -Name IsCIEnv -ErrorAction SilentlyContinue)
            {
                Trace-Execution "CI Environment. Downgrade cluster quorum result severity to warning"
                $AddNodeResult | Where-Object Name -eq 'AzStackHci_AddNode_Quorum_Check' | ForEach-Object {
                    $_.Severity = 'WARNING'
            ...
            if ([EnvironmentValidator]::ParseResult.OverloadDefinitions -match 'EceInterfaceParameters')
            {
                ...
                return [EnvironmentValidator]::ParseResult($AddNodeResult, 'AddNode', $FailFast, $Parameters)
            }
            else
            {
                Trace-Execution "ParseResult method does not support EceInterfaceParameters argument"
                return [EnvironmentValidator]::ParseResult($AddNodeResult, 'AddNode', $FailFast)
            }
        }
        else
        {
    ...
    finally
    {
        ...
        # Remove any PSSession created by this module
        if ($PsSession)
        {
...
Export-ModuleMember -Function Test-AzStackHciAddNode -Variable MetaData
# SIG # Begin signature block
# MIIncQYJKoZIhvcNAQcCoIInYjCCJ14CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC